...
describe('MimocodeAuxQueryRunner', () => {
  let mockConnection: {
    cancel: jest.Mock;
    dispose: jest.Mock;
  ...
  beforeEach(() => {
    ...
    mockPrepareMimocodeLaunchArtifacts.mockResolvedValue({
      configPath: '/tmp/grimoire-mimocode-aux/config.json',
      configContent: '{"default_agent":"grimoire-aux-passive"}\n',
      databasePath: null,
      launchKey: 'launch-key',
      systemPromptPath: '/tmp/grimoire-mimocode-aux/system.md',
    });
  });

  it('launches an auxiliary ACP session and streams assistant text', async () => {
    const runner = new MimocodeAuxQueryRunner(createMockPlugin(), {
    ...
    expect(mockPrepareMimocodeLaunchArtifacts).toHaveBeenCalledWith(expect.objectContaining({
      artifactsSubdir: 'mimocode/auxiliary/title-gen',
  ...
  it('does not pass the workspace path through auxiliary MiMoCode CLI arguments', async () => {
    const plugin = createMockPlugin();
