...
export class McpTestModal extends Modal {
  ...
  private renderLoading() {
    ...
    const loadingEl = this.contentEl_.createDiv({ cls: 'grimoire-mcp-test-loading' });
  ...
  private render() {
    ...
    const statusEl = this.contentEl_.createDiv({ cls: 'grimoire-mcp-test-status' });

    const iconEl = statusEl.createSpan({ cls: 'grimoire-mcp-test-icon' });
    ...
    if (this.result.error) {
      const errorEl = this.contentEl_.createDiv({ cls: 'grimoire-mcp-test-error' });
    ...
    if (this.result.tools.length > 0 && this.onToolToggle) {
      this.toggleAllBtn = buttonContainer.createEl('button', {
        cls: 'grimoire-mcp-toggle-all-btn',
      });
  ...
  private renderTool(container: HTMLElement, tool: McpTool) {
    const toolEl = container.createDiv({ cls: 'grimoire-mcp-test-tool' });

    const headerEl = toolEl.createDiv({ cls: 'grimoire-mcp-test-tool-header' });

    const iconEl = headerEl.createSpan({ cls: 'grimoire-mcp-test-tool-icon' });
    setIcon(iconEl, 'wrench');

    const nameEl = headerEl.createSpan({ cls: 'grimoire-mcp-test-tool-name' });
    nameEl.setText(tool.name);

    const toggleEl = headerEl.createDiv({ cls: 'grimoire-mcp-test-tool-toggle' });
    ...
    if (tool.description) {
      const descEl = toolEl.createDiv({ cls: 'grimoire-mcp-test-tool-desc' });
  ...
  private updateToolState(toolEl: HTMLElement, enabled: boolean) {
    toolEl.toggleClass('grimoire-mcp-test-tool-disabled', !enabled);
  }
