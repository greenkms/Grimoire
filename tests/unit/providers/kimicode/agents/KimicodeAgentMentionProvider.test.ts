import { KimicodeAgentMentionProvider } from '@/providers/kimicode/agents/KimicodeAgentMentionProvider';
import type { KimicodeAgentStorage } from '@/providers/kimicode/storage/KimicodeAgentStorage';
...
function makeMockStorage(agents: KimicodeAgentDefinition[] = []): KimicodeAgentStorage {
  return { loadAll: async () => agents } as unknown as KimicodeAgentStorage;
}

describe('KimicodeAgentMentionProvider', () => {
  it('returns an empty array before loadAgents is called', () => {
    const provider = new KimicodeAgentMentionProvider(makeMockStorage([makeAgent()]));
    expect(provider.searchAgents('')).toEqual([]);
  });

  it('returns only explicit subagents after load', async () => {
    const provider = new KimicodeAgentMentionProvider(makeMockStorage([
