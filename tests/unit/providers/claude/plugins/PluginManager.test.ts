...
describe('PluginManager', () => {
  ...
  describe('loadPlugins', () => {
    it('returns empty array when no installed_plugins.json exists', async () => {
      ...
      const manager = new PluginManager(vaultPath);
  ...
  describe('plugin enablement API', () => {
    it('does not expose plugin enablement mutation methods', () => {
      const manager = new PluginManager(vaultPath);

      expect('togglePlugin' in manager).toBe(false);
      expect('enablePlugin' in manager).toBe(false);
      expect('disablePlugin' in manager).toBe(false);
    });
  });
