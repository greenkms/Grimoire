...
import {
  createKimicodeAgentPersistenceKey,
  KIMICODE_AGENT_PATH,
  KIMICODE_AGENTS_PATH,
  KimicodeAgentStorage,
  parseKimicodeAgentMarkdown,
  parseKimicodeAgentPersistenceKey,
  serializeKimicodeAgentMarkdown,
} from '@/providers/kimicode/storage/KimicodeAgentStorage';
...
describe('parseKimicodeAgentMarkdown', () => {
  it('parses required fields and derives the name from the file path', () => {
    const result = parseKimicodeAgentMarkdown(BASIC_MARKDOWN, '.kimicode/agent/review.md');
    ...
    expect(result!.persistenceKey).toBe(
      createKimicodeAgentPersistenceKey({ filePath: '.kimicode/agent/review.md' }),
    );
  });

  it('supports nested agent paths and maxSteps fallback', () => {
    const result = parseKimicodeAgentMarkdown(`---
description: "Explores bugs."
maxSteps: 7
---
Trace the bug carefully.
`, '.kimicode/agents/nested/explorer.md');

    expect(result).not.toBeNull();
    expect(result!.name).toBe('nested/explorer');
    expect(result!.steps).toBe(7);
    expect(result!.persistenceKey).toBe(
      createKimicodeAgentPersistenceKey({ filePath: '.kimicode/agents/nested/explorer.md' }),
    );
  });

  it('preserves supported optional fields and unknown frontmatter', () => {
    const result = parseKimicodeAgentMarkdown(FULL_MARKDOWN, '.kimicode/agents/reviewer.md');
  ...
  it('returns null when required fields are missing', () => {
    expect(parseKimicodeAgentMarkdown('---\nmode: subagent\n---\nPrompt', 'review.md')).toBeNull();
  });
});

describe('serializeKimicodeAgentMarkdown', () => {
  it('serializes supported fields and round-trips through parse', () => {
    const original: KimicodeAgentDefinition = {
    ...
    const markdown = serializeKimicodeAgentMarkdown(original);
    const reparsed = parseKimicodeAgentMarkdown(markdown, '.kimicode/agents/nested/reviewer.md');
...
describe('KimicodeAgentStorage', () => {
  describe('loadAll', () => {
    it('loads markdown agents recursively from both supported Kimi Code roots', async () => {
      const adapter = createMockAdapter({
        '.kimicode/agent/review.md': BASIC_MARKDOWN,
        '.kimicode/agents/nested/explorer.md': `---
description: "Explores bugs."
mode: all
---
Trace the bug carefully.
`,
        '.kimicode/agent/README.txt': 'ignore me',
      });

      const storage = new KimicodeAgentStorage(adapter);
      const agents = await storage.loadAll();

      expect(agents.map((agent) => agent.name).sort()).toEqual(['nested/explorer', 'review']);
      expect(adapter.listFilesRecursive).toHaveBeenCalledWith(KIMICODE_AGENTS_PATH);
      expect(adapter.listFilesRecursive).toHaveBeenCalledWith(KIMICODE_AGENT_PATH);
    });

    it('prefers the singular root when duplicate agent names exist in both roots', async () => {
      const adapter = createMockAdapter({
        '.kimicode/agents/review.md': `---
description: "Plural root"
mode: subagent
---
Plural prompt.
`,
        '.kimicode/agent/review.md': `---
...
`,
      ...
      const agent = await storage.load({
        ...
        description: '',
        prompt: '',
        persistenceKey: createKimicodeAgentPersistenceKey({ filePath: '.kimicode/agents/nested/explorer.md' }),
      });
  ...
  describe('save', () => {
    it('writes new nested agents to the singular Kimi Code root', async () => {
      const adapter = createMockAdapter({});
      const storage = new KimicodeAgentStorage(adapter);
      ...
      expect(adapter.ensureFolder).toHaveBeenCalledWith('.kimicode/agent/nested');
      expect(adapter.write).toHaveBeenCalledWith(
        '.kimicode/agent/nested/reviewer.md',
    ...
    it('preserves the existing plural-root backing file when the name is unchanged', async () => {
      ...
      await storage.save({
        ...
        description: 'Reviews code.',
        prompt: 'Review carefully.',
        persistenceKey: createKimicodeAgentPersistenceKey({ filePath: '.kimicode/agents/legacy/reviewer.md' }),
      });

      expect(adapter.write).toHaveBeenCalledWith(
        '.kimicode/agents/legacy/reviewer.md',
        expect.any(String),
      );
    ...
    it('preserves the existing singular-root backing file when the name is unchanged', async () => {
      ...
      await storage.save({
        ...
        description: 'Reviews code.',
        prompt: 'Review carefully.',
        persistenceKey: createKimicodeAgentPersistenceKey({ filePath: '.kimicode/agent/legacy/reviewer.md' }),
      });

      expect(adapter.write).toHaveBeenCalledWith(
        '.kimicode/agent/legacy/reviewer.md',
        expect.any(String),
      );
      expect(adapter.delete).not.toHaveBeenCalled();
    });
    ...
    it('renames the backing file when the agent name changes', async () => {
      ...
      expect(adapter.write).toHaveBeenCalledWith(
        '.kimicode/agent/reviewer.md',
        expect.any(String),
      );
      expect(adapter.delete).toHaveBeenCalledWith('.kimicode/agent/legacy/reviewer.md');
    });
  });
  ...
  describe('delete', () => {
    it('deletes the resolved nested file', async () => {
      ...
      await storage.delete({
        ...
        description: '',
        prompt: '',
        persistenceKey: createKimicodeAgentPersistenceKey({ filePath: '.kimicode/agent/nested/reviewer.md' }),
      });

      expect(adapter.delete).toHaveBeenCalledWith('.kimicode/agent/nested/reviewer.md');
    });
  });
});

describe('Kimicode agent persistence keys', () => {
  it('round-trips nested file identity for the singular Kimi Code root', () => {
    const key = createKimicodeAgentPersistenceKey({ filePath: '.kimicode/agent/nested/reviewer.md' });

    expect(parseKimicodeAgentPersistenceKey(key)).toEqual({ filePath: '.kimicode/agent/nested/reviewer.md' });
  });

  it('parses direct singular-root storage paths for backward compatibility', () => {
    expect(parseKimicodeAgentPersistenceKey('.kimicode/agent/nested/reviewer.md')).toEqual({
      filePath: '.kimicode/agent/nested/reviewer.md',
    });
  });

  it('parses direct plural-root storage paths for backward compatibility', () => {
    expect(parseKimicodeAgentPersistenceKey('.kimicode/agents/nested/reviewer.md')).toEqual({
      filePath: '.kimicode/agents/nested/reviewer.md',
    });
  });

  it('parses legacy encoded relative paths produced by the initial implementation', () => {
    expect(parseKimicodeAgentPersistenceKey('kimicode-agent:nested%2Freviewer.md')).toEqual({
      filePath: '.kimicode/agents/nested/reviewer.md',
    });
  });
});
