...
describe('ClaudeChatRuntime', () => {
  ...
  describe('persistent query dynamic updates', () => {
    ...
    it('updates permission mode via setPermissionMode when going from Auto-approve to normal', async () => {
      // Start in Auto-approve mode
      mockPlugin.settings.permissionMode = 'full_access';
      service = new ClaudeChatRuntime(mockPlugin, createMockMcpManager());
      ...
      const response = getLastResponse();
      // Should call setPermissionMode for Auto-approve -> normal transition
      expect(response?.setPermissionMode).toHaveBeenCalledWith('default');
    });

    it('updates permission mode via setPermissionMode when going from normal to Auto-approve', async () => {
      ...
      const response = getLastResponse();
      // Should call setPermissionMode for normal -> Auto-approve transition (no restart needed)
