...
describe('KimicodeRuntimeCommandLoader', () => {
  ...
  it('uses an isolated in-memory session for blank-tab command warmup', async () => {
    ...
    const syncSpy = jest.spyOn(KimicodeChatRuntime.prototype, 'syncConversationState').mockImplementation(() => {});
    const ensureReadySpy = jest.spyOn(KimicodeChatRuntime.prototype, 'ensureReady').mockResolvedValue(true);
    const getSupportedCommandsSpy = jest.spyOn(KimicodeChatRuntime.prototype, 'getSupportedCommands').mockResolvedValue(commands);
    const cleanupSpy = jest.spyOn(KimicodeChatRuntime.prototype, 'cleanup').mockImplementation(() => {});
