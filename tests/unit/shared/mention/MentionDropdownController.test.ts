...
describe('MentionDropdownController', () => {
  ...
  describe('tool mentions', () => {
    it('shows vault search when typing @vau', () => {
      inputEl.value = '@vau';
    ...
    it('localizes the vault search tool label', () => {
      setLocale('de');
      ...
      expect(renderOptions.items).toEqual(expect.arrayContaining([
        expect.objectContaining({
          ...
          id: 'vault',
          name: 'Tresorsuche',
          description: 'Tresornotizen für diesen Durchlauf durchsuchen',
        }),
      ]));
    });

    it('inserts @vault and calls the tool callback when selected', () => {
      const onToolMentionSelect = jest.fn();
      const localCallbacks = createMockCallbacks({ onToolMentionSelect });
      ...
      localInput.value = '@vault';
      localInput.selectionStart = 6;
      localInput.selectionEnd = 6;
      ...
      expect(localInput.value).toBe('@vault ');
      expect(onToolMentionSelect).toHaveBeenCalledWith('vault');
  ...
  describe('handleInputChange', () => {
    ...
    it('keeps vault suggestions open when the mention query contains spaces', () => {
  ...
  describe('vault folder mentions', () => {
    ...
    it('renders vault folder text in @path/ format', () => {
      ...
      const nameEl = itemEl.querySelector('.grimoire-mention-name-folder');
