...
describe('KimicodeAuxQueryRunner', () => {
  let mockConnection: {
    cancel: jest.Mock;
    dispose: jest.Mock;
  ...
  beforeEach(() => {
    ...
    mockPrepareKimicodeLaunchArtifacts.mockResolvedValue({
      configPath: '/tmp/grimoire-kimicode-aux/config.json',
      configContent: '{"default_agent":"grimoire-aux-passive"}\n',
      databasePath: null,
      launchKey: 'launch-key',
      systemPromptPath: '/tmp/grimoire-kimicode-aux/system.md',
    });
  });

  it('launches an auxiliary ACP session and streams assistant text', async () => {
    const runner = new KimicodeAuxQueryRunner(createMockPlugin(), {
    ...
    expect(mockPrepareKimicodeLaunchArtifacts).toHaveBeenCalledWith(expect.objectContaining({
      artifactsSubdir: 'kimicode/auxiliary/title-gen',
  ...
  it('uses an explicit encoded Kimi Code model override from the active chat tab', async () => {
  ...
  it('rejects aux reads outside the workspace root', () => {
    ...
    expect(() => (runner as any).resolveSessionPath('session-1', '/tmp/outside.md')).toThrow(
      'Kimi Code aux read access is limited to the current workspace.',
    );
