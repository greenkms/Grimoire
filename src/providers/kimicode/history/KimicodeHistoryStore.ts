...
import {
  ...
  normalizeKimicodeToolUseResult,
} from '../normalization/kimicodeToolNormalization';
...
export function mapKimicodeMessages(messages: StoredMessage[]): ChatMessage[] {
...
function buildAssistantToolCalls(parts: StoredRow[]): ToolCallInfo[] {
  return parts.flatMap((part) => {
    ...
    const input = normalizeKimicodeToolInput(rawName, getObject(state?.input) ?? {});
...
async function loadKimicodeSessionRows(
  databasePath: string,
  sessionId: string,
