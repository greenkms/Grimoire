...
export class McpStorage {
  ...
  async load(): Promise<ManagedMcpServer[]> {
    try {
      ...
      const grimoireMeta = file._grimoire?.servers ?? {};
  ...
  async save(servers: ManagedMcpServer[]): Promise<void> {
    ...
    for (const server of servers) {
      ...
      // Only store Grimoire metadata if different from defaults
      const meta: {
        enabled?: boolean;
      ...
      if (Object.keys(meta).length > 0) {
        grimoireServers[server.name] = meta;
      }
    }
    ...
    const existingGrimoire =
      existing && typeof existing._grimoire === 'object'
        ? (existing._grimoire as Record<string, unknown>)
        : null;

    if (Object.keys(grimoireServers).length > 0) {
      file._grimoire = { ...(existingGrimoire ?? {}), servers: grimoireServers };
    } else if (existingGrimoire) {
      const rest = { ...existingGrimoire };
      delete rest.servers;
      if (Object.keys(rest).length > 0) {
        file._grimoire = rest;
      } else {
        delete file._grimoire;
      }
    } else {
      delete file._grimoire;
    }
