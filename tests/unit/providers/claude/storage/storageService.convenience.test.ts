...
describe('StorageService convenience methods', () => {
  ...
  const grimoireSettingsJson = JSON.stringify({
    userName: 'Test',
    model: 'haiku',
  ...
  describe('updateGrimoireSettings', () => {
    it('updates partial Grimoire settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.grimoire/grimoire-settings.json': grimoireSettingsJson,
        },
      });
      const storage = new StorageService(plugin);
      await storage.initialize();

      await storage.updateGrimoireSettings({ userName: 'NewUser' });

      const saved = JSON.parse(files.get('.grimoire/grimoire-settings.json')!) as Record<string, unknown>;
      expect(saved.userName).toBe('NewUser');
    });
  });

  describe('saveGrimoireSettings', () => {
    it('saves full Grimoire settings', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.grimoire/grimoire-settings.json': grimoireSettingsJson,
        },
      ...
      const existing = await storage.loadGrimoireSettings();
      existing.userName = 'FullSave';
      await storage.saveGrimoireSettings(existing);
  ...
  describe('loadGrimoireSettings', () => {
    it('loads Grimoire settings', async () => {
      const { plugin } = createMockPlugin({
        initialFiles: {
          '.grimoire/grimoire-settings.json': grimoireSettingsJson,
        },
      ...
      const settings = await storage.loadGrimoireSettings();
      expect(settings.userName).toBe('Test');
      expect(settings.model).toBe('haiku');
    });

    it('ignores old .claude settings during initialization', async () => {
      const { plugin, files } = createMockPlugin({
        initialFiles: {
          '.claude/grimoire-settings.json': grimoireSettingsJson,
        },
      });
      const storage = new StorageService(plugin);
      ...
      expect(settings.grimoire.userName).toBe('');
      expect(files.get('.grimoire/grimoire-settings.json')).toBeUndefined();
      expect(files.has('.claude/grimoire-settings.json')).toBe(true);
    });
  });
  ...
  describe('getTabManagerState', () => {
    ...
    it('preserves blank-tab draft settings and orchestrator mode', async () => {
      const state = {
        openTabs: [{
          tabId: 'tab-1',
          conversationId: null,
