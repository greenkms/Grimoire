...
describe('utils.ts', () => {
  ...
  describe('expandHomePath', () => {
    const envKey = 'GRIMOIRE_TEST_PATH';
    const envValue = path.join(os.tmpdir(), 'grimoire-env');
    ...
    it('should leave unknown environment variables untouched', () => {
      expect(expandHomePath('%GRIMOIRE_MISSING_VAR%')).toBe('%GRIMOIRE_MISSING_VAR%');
      expect(expandHomePath('$GRIMOIRE_MISSING_VAR')).toBe('$GRIMOIRE_MISSING_VAR');
    });
  });
  ...
  describe('normalizePathForFilesystem', () => {
    ...
    it('expands environment variables before filesystem use', () => {
      const envKey = 'GRIMOIRE_FS_TEST_PATH';
    ...
    it('handles chained home and environment variable expansions', () => {
      const envKey = 'GRIMOIRE_TEST_SUBDIR';
      const originalValue = process.env[envKey];
