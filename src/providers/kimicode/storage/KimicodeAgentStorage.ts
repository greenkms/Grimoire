...
import {
  KIMICODE_AGENT_KNOWN_KEYS,
  type KimicodeAgentDefinition,
} from '../types/agent';

export const KIMICODE_AGENT_PATH = '.kimicode/agent';
export const KIMICODE_AGENTS_PATH = '.kimicode/agents';
const KIMICODE_AGENT_SCAN_PATHS = [
  KIMICODE_AGENTS_PATH,
  KIMICODE_AGENT_PATH,
] as const;
const KIMICODE_DEFAULT_AGENT_SAVE_PATH = KIMICODE_AGENT_PATH;
const KIMICODE_AGENT_PERSISTENCE_PREFIX = 'kimicode-agent';

export interface KimicodeAgentLocation {
  filePath: string;
}

export function createKimicodeAgentPersistenceKey(
  location: KimicodeAgentLocation,
): string {
  return `${KIMICODE_AGENT_PERSISTENCE_PREFIX}:${encodeURIComponent(normalizeVaultPath(location.filePath))}`;
}

export function parseKimicodeAgentPersistenceKey(
  persistenceKey?: string,
): KimicodeAgentLocation | null {
  if (!persistenceKey) {
    return null;
  }
  ...
  return decoded.endsWith('.md')
    ? { filePath: `${KIMICODE_AGENTS_PATH}/${decoded}` }
    : null;
}

export class KimicodeAgentStorage {
  constructor(
    private vaultAdapter: Pick<VaultFileAdapter, 'exists' | 'read' | 'write' | 'delete' | 'listFilesRecursive' | 'ensureFolder'>,
  ) {}

  async loadAll(): Promise<KimicodeAgentDefinition[]> {
    return this.scanAdapter(this.vaultAdapter);
  }

  async load(agent: KimicodeAgentDefinition): Promise<KimicodeAgentDefinition | null> {
  ...
  async save(agent: KimicodeAgentDefinition, previous?: KimicodeAgentDefinition | null): Promise<void> {
  ...
  async delete(agent: KimicodeAgentDefinition): Promise<void> {
    const filePath = this.resolveCurrentPath(agent);
    await this.vaultAdapter.delete(filePath);
  }

  private resolveCurrentPath(agent: KimicodeAgentDefinition): string {
  ...
  private resolveTargetPath(
    agent: KimicodeAgentDefinition,
    previous?: KimicodeAgentDefinition | null,
  ): string {
  ...
  ): Promise<KimicodeAgentDefinition[]> {
    const agentsByName = new Map<string, KimicodeAgentDefinition>();
...
export function parseKimicodeAgentMarkdown(
  content: string,
  filePath: string,
  ...
  const result: KimicodeAgentDefinition = {
    name,
    description,
...
export function serializeKimicodeAgentMarkdown(agent: KimicodeAgentDefinition): string {
  const lines: string[] = ['---'];
...
function normalizeMode(value: unknown): KimicodeAgentDefinition['mode'] | undefined {
...
function isSupportedAgentFilePath(filePath: string): boolean {
  return KIMICODE_AGENT_SCAN_PATHS.some((rootPath) => filePath.startsWith(`${rootPath}/`))
