/**
 * Grimoire - Claude Agent SDK wrapper
 *
...
import { claudePlanUsageStore } from '../app/ClaudePlanUsageStore';
...
function buildClaudePlanUsageDebugData(message: SDKMessage): Record<string, unknown> | null {
  if (!isRecord(message)) {
    return null;
  }
  ...
  if (message.type === 'rate_limit_event') {
    ...
    const rateLimitInfoFields = info ? Object.keys(info).sort().join(',') : '';
    ...
    const hasUtilization = typeof info?.utilization === 'number' && Number.isFinite(info.utilization);
    const hasResetsAt = hasResetValue(info?.resetsAt);
    const hasOverageResetsAt = hasResetValue(info?.overageResetsAt);
    const hasReset = hasResetsAt || hasOverageResetsAt;
    const acceptedWithPct = isClaudeRateLimitType(rateLimitType)
      && (hasUtilization || status === 'rejected')
      && hasReset;
    const acceptedResetOnly = isClaudeRateLimitType(rateLimitType)
      && !acceptedWithPct
      && hasReset;
    const accepted = acceptedWithPct || acceptedResetOnly;

    return {
      accepted,
      hasOverageResetsAt,
      hasRateLimitInfo: !!info,
      hasResetsAt,
      hasUtilization,
      messageType,
      rateLimitInfoFields,
      ...(rateLimitType ? { rateLimitType } : {}),
      reason: acceptedWithPct ? 'accepted' : acceptedResetOnly ? 'accepted_reset_only' : 'incomplete_rate_limit_info',
      ...(status ? { status } : {}),
      usageKind: accepted ? 'quota' : 'none',
    };
  }
  ...
  return {
    ...
    messageType,
    modelUsageCount,
    reason: hasCost ? 'accepted' : 'missing_cost',
    usageKind: hasCost ? 'spend' : 'none',
  };
}

function isClaudeRateLimitType(value: string | null): boolean {
...
function hasResetValue(value: unknown): boolean {
...
export class ClaudeChatRuntime implements ChatRuntime {
   ...
   */
  private getScopedSettings(): GrimoireSettings {
  ...
  private async routeMessage(message: SDKMessage): Promise<void> {
    ...
    this.recordPlanUsageMessage(message);
    ...
    if (shouldUsePersistent) {
      ...
      if (!this.persistentQuery && !this.shuttingDown) {
        await this.startPersistentQuery(
          ...
          undefined,
          effectiveQueryOptions?.orchestratorMode,
        );
      }
  ...
  private recordPlanUsageMessage(message: SDKMessage): void {
    const changed = claudePlanUsageStore.recordSdkMessage(message);
    const debugData = buildClaudePlanUsageDebugData(message);
    if (debugData) {
      this.plugin.recordDebugLog?.({
        ...
        event: 'sdk.usage_message',
        level: 'debug',
        scope: 'claude.usage',
      });
    }
    if (changed) {
      this.refreshPlanUsageSelectors();
    }
  }

  private refreshPlanUsageSelectors(): void {
  ...
  private resolveSDKPermissionMode(mode: PermissionMode): SDKPermissionMode {
    return QueryOptionsBuilder.resolveClaudeSdkPermissionMode(mode);
  }
}
