...
function diffToHtml(ops: DiffOp[]): string {
  return ops
    .map((op) => {
      ...
      switch (op.type) {
        case 'delete':
          return `<span class="grimoire-diff-del">${escaped}</span>`;
        case 'insert':
          return `<span class="grimoire-diff-ins">${escaped}</span>`;
        default:
          return escaped;
      }
    })
...
describe('InlineEditModal - Word-level Diff', () => {
  ...
  describe('diffToHtml', () => {
    ...
    it('should wrap deleted text with del class', () => {
      ...
      expect(html).toContain('grimoire-diff-del');
    ...
    it('should wrap inserted text with ins class', () => {
      ...
      expect(html).toContain('grimoire-diff-ins');
