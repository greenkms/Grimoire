...
export function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  const viewHeight = textarea.closest('.grimoire-container')?.clientHeight ?? window.innerHeight;
  const maxHeight = calculateTextareaMaxHeight(viewHeight);

  textarea.setCssProps({
    '--grimoire-textarea-min-height': `${TEXTAREA_BASE_MIN_HEIGHT}px`,
    '--grimoire-textarea-max-height': `${maxHeight}px`,
  });
  ...
  textarea.setCssProps({
    '--grimoire-textarea-min-height': `${minHeight}px`,
    '--grimoire-textarea-max-height': `${maxHeight}px`,
  });
}
