...
export class KimicodeAuxQueryRunner implements AuxQueryRunner {
  ...
  async query(config: AuxQueryConfig, prompt: string): Promise<string> {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Kimi Code request failed';
  ...
  private resolveSelectedRawModel(explicitModel?: string): string | undefined {
    ...
    if (explicitModel) {
      ...
      return kimicodeChatUIConfig.ownsModel(trimmed, projectedSettings)
    ...
    const selectedModel = typeof projectedSettings.model === 'string'
      ...
      : '';
    return kimicodeChatUIConfig.ownsModel(selectedModel, projectedSettings)
