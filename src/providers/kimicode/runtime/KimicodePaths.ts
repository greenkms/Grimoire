...
const KIMICODE_APP_NAME = 'kimicode';
const DEFAULT_DATABASE_NAME = 'kimicode.db';
const DATABASE_NAME_PATTERN = /^kimicode(?:-[a-z0-9._-]+)?\.db$/i;

export function resolveKimicodeDataDir(
  env: NodeJS.ProcessEnv = process.env,
): string {
  const xdgDataHome = env.XDG_DATA_HOME?.trim();
  if (xdgDataHome) {
    return path.join(xdgDataHome, KIMICODE_APP_NAME);
  }

  const home = env.HOME || os.homedir();
  if (process.platform === 'win32') {
    const appData = env.APPDATA || env.LOCALAPPDATA || path.join(home, 'AppData', 'Roaming');
    return path.join(appData, KIMICODE_APP_NAME);
  }

  return path.join(home, '.local', 'share', KIMICODE_APP_NAME);
}

export function resolveKimicodeDatabasePath(
  env: NodeJS.ProcessEnv = process.env,
): string | null {
  const override = env.KIMICODE_DB?.trim();
  if (override) {
    if (override === ':memory:' || path.isAbsolute(override)) {
      return override;
    }
    return path.join(resolveKimicodeDataDir(env), override);
  }
...
export function resolveExistingKimicodeDatabasePath(
...
function getKimicodeDatabasePathCandidates(
  ...
  const dataDirs = [
    ...
    path.join(home, 'Library', 'Application Support', KIMICODE_APP_NAME),
  ];
