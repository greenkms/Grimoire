...
import { getCodexModelDiscoveryState } from './modelDiscoveryState';
...
export function getConfiguredEnvCustomModel(settings: Record<string, unknown>): string | null {
  ...
  return modelId && !getBaseCodexModelSet(settings).has(modelId) ? modelId : null;
}
...
export function getCodexModelOptions(settings: Record<string, unknown>): ProviderUIOption[] {
  const models = getBaseCodexModelOptions(settings);
...
function getBaseCodexModelOptions(settings: Record<string, unknown>): ProviderUIOption[] {
  const discoveredModels = getCodexModelDiscoveryState(settings).discoveredModels;
...
function getBaseCodexModelSet(settings: Record<string, unknown>): Set<string> {
  return new Set(getBaseCodexModelOptions(settings).map(model => model.value));
}
