...
export class SlashCommandModal extends Modal {
  ...
  onOpen() {
    ...
    const updateSkillOnlyFields = () => {
      ...
      disableUserSetting.settingEl.toggleClass('grimoire-hidden', !isSkillType);
      if (!isSkillType) {
      ...
      .addToggle(toggle => {
        toggle.setValue(contextValue === 'fork')
          .onChange(value => {
            ...
            agentSetting.settingEl.toggleClass('grimoire-hidden', !value);
          });
      });
...
export class SlashCommandSettings {
  ...
  private render(): void {
    ...
    headerEl.createSpan({ text: t('settings.slashCommands.name'), cls: 'grimoire-sp-label' });
  ...
  private renderCommandItem(listEl: HTMLElement, cmd: ProviderCommandEntry): void {
    ...
    if (cmd.argumentHint) {
      const hintEl = headerRow.createSpan({ cls: 'grimoire-slash-item-hint' });
