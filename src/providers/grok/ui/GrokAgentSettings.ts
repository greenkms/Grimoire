...
export function findGrokAgentNameConflict(
  agents: GrokAgentDefinition[],
  name: string,
...
class GrokAgentModal extends Modal {
  private existing: GrokAgentDefinition | null;
  private allAgents: GrokAgentDefinition[];
  private onSave: (agent: GrokAgentDefinition) => Promise<void>;

  constructor(
    app: App,
    existing: GrokAgentDefinition | null,
    allAgents: GrokAgentDefinition[],
    onSave: (agent: GrokAgentDefinition) => Promise<void>,
  ) {
    super(app);
  ...
  onOpen() {
    ...
    saveBtn.addEventListener('click', () => {
      ...
      const nameError = validateGrokAgentName(name);
      if (nameError) {
      ...
      const duplicate = findGrokAgentNameConflict(
        this.allAgents,
        name,
      ...
      const agent: GrokAgentDefinition = {
        name,
        description,
        prompt,
        mode: 'subagent',
...
export class GrokAgentSettings {
  private containerEl: HTMLElement;
  private storage: GrokAgentStorage;
  ...
  private renderItem(listEl: HTMLElement, agent: GrokAgentDefinition): void {
  ...
  private openModal(existing: GrokAgentDefinition | null): void {
    if (!this.app) return;

    const modal = new GrokAgentModal(
      this.app,
      existing,
      this.agents,
      async (agent) => {
