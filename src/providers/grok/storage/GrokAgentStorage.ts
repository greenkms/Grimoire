...
import {
  GROK_AGENT_KNOWN_KEYS,
  type GrokAgentDefinition,
} from '../types/agent';

export const GROK_AGENT_PATH = '.grok/agent';
export const GROK_AGENTS_PATH = '.grok/agents';
const GROK_AGENT_SCAN_PATHS = [
  GROK_AGENTS_PATH,
  GROK_AGENT_PATH,
] as const;
const GROK_DEFAULT_AGENT_SAVE_PATH = GROK_AGENT_PATH;
const GROK_AGENT_PERSISTENCE_PREFIX = 'grok-agent';

export interface GrokAgentLocation {
  filePath: string;
}

export function createGrokAgentPersistenceKey(
  location: GrokAgentLocation,
): string {
  return `${GROK_AGENT_PERSISTENCE_PREFIX}:${encodeURIComponent(normalizeVaultPath(location.filePath))}`;
}

export function parseGrokAgentPersistenceKey(
  persistenceKey?: string,
): GrokAgentLocation | null {
  if (!persistenceKey) {
    return null;
  }
  ...
  if (prefix !== GROK_AGENT_PERSISTENCE_PREFIX || !encodedRelativePath) {
    return null;
  }
  ...
  return decoded.endsWith('.md')
    ? { filePath: `${GROK_AGENTS_PATH}/${decoded}` }
    : null;
}

export class GrokAgentStorage {
  constructor(
    private vaultAdapter: Pick<VaultFileAdapter, 'exists' | 'read' | 'write' | 'delete' | 'listFilesRecursive' | 'ensureFolder'>,
  ) {}

  async loadAll(): Promise<GrokAgentDefinition[]> {
    return this.scanAdapter(this.vaultAdapter);
  }

  async load(agent: GrokAgentDefinition): Promise<GrokAgentDefinition | null> {
    ...
    try {
      ...
      return parseGrokAgentMarkdown(content, filePath);
    } catch {
      return null;
    }
  }

  async save(agent: GrokAgentDefinition, previous?: GrokAgentDefinition | null): Promise<void> {
    ...
    const content = serializeGrokAgentMarkdown(agent);
  ...
  async delete(agent: GrokAgentDefinition): Promise<void> {
    const filePath = this.resolveCurrentPath(agent);
    await this.vaultAdapter.delete(filePath);
  }

  private resolveCurrentPath(agent: GrokAgentDefinition): string {
    const persistedLocation = parseGrokAgentPersistenceKey(agent.persistenceKey);
    if (persistedLocation) {
      return persistedLocation.filePath;
    }

    return `${GROK_DEFAULT_AGENT_SAVE_PATH}/${agent.name}.md`;
  }

  private resolveTargetPath(
    agent: GrokAgentDefinition,
    previous?: GrokAgentDefinition | null,
  ): string {
  ...
  ): Promise<GrokAgentDefinition[]> {
    const agentsByName = new Map<string, GrokAgentDefinition>();

    for (const rootPath of GROK_AGENT_SCAN_PATHS) {
      try {
        ...
        for (const filePath of files) {
          ...
          try {
            ...
            const agent = parseGrokAgentMarkdown(content, filePath);
            if (!agent) continue;
...
export function parseGrokAgentMarkdown(
  content: string,
  filePath: string,
): GrokAgentDefinition | null {
  ...
  const result: GrokAgentDefinition = {
    name,
    description,
    prompt: parsed.body.trim(),
    persistenceKey: createGrokAgentPersistenceKey({
  ...
  for (const [key, value] of Object.entries(frontmatter)) {
    if (!GROK_AGENT_KNOWN_KEYS.has(key)) {
...
export function serializeGrokAgentMarkdown(agent: GrokAgentDefinition): string {
  const lines: string[] = ['---'];
...
function normalizeMode(value: unknown): GrokAgentDefinition['mode'] | undefined {
...
function isSupportedAgentFilePath(filePath: string): boolean {
  return GROK_AGENT_SCAN_PATHS.some((rootPath) => filePath.startsWith(`${rootPath}/`))
