import type { KimicodeDiscoveredModel, KimicodeThinkingOptionsByModel } from '../models';
import type { KimicodeMode } from '../modes';
...
export function sameModes(left: KimicodeMode[], right: KimicodeMode[]): boolean {
  if (left.length !== right.length) {
    return false;
  }
...
export function sameDiscoveredModels(
  left: KimicodeDiscoveredModel[],
  right: KimicodeDiscoveredModel[],
): boolean {
  if (left.length !== right.length) {
    return false;
  }
...
export function sameThinkingOptionsByModel(
  left: KimicodeThinkingOptionsByModel,
  right: KimicodeThinkingOptionsByModel,
): boolean {
  const leftEntries = Object.entries(left);
  if (leftEntries.length !== Object.keys(right).length) {
    return false;
