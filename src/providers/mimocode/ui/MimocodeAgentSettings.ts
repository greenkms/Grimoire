...
const MIMOCODE_AGENT_INVALID_SEGMENT_PATTERN = /[<>:"\\|?*]/;

export function validateMimocodeAgentName(name: string): string | null {
...
export function findMimocodeAgentNameConflict(
  agents: MimocodeAgentDefinition[],
  name: string,
...
class MimocodeAgentModal extends Modal {
  private existing: MimocodeAgentDefinition | null;
  private allAgents: MimocodeAgentDefinition[];
  private onSave: (agent: MimocodeAgentDefinition) => Promise<void>;

  constructor(
    app: App,
    existing: MimocodeAgentDefinition | null,
    allAgents: MimocodeAgentDefinition[],
    onSave: (agent: MimocodeAgentDefinition) => Promise<void>,
  ) {
    super(app);
  ...
  onOpen() {
    ...
    saveBtn.addEventListener('click', () => {
      ...
      const agent: MimocodeAgentDefinition = {
        name,
        description,
        prompt,
        mode: 'subagent',
...
export class MimocodeAgentSettings {
  private containerEl: HTMLElement;
  private storage: MimocodeAgentStorage;
  ...
  private renderItem(listEl: HTMLElement, agent: MimocodeAgentDefinition): void {
  ...
  private openModal(existing: MimocodeAgentDefinition | null): void {
    if (!this.app) return;
