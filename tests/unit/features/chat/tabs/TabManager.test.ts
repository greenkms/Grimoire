...
describe('TabManager - Tab Lifecycle', () => {
  ...
  describe('createTab', () => {
    ...
    it('should forward blank-tab draft settings changes', async () => {
      callbacks.onTabDraftSettingsChanged = jest.fn();
      const manager = createManager({
        callbacks,
        tabFactory: () => createMockTabData({ id: 'tab-1' }),
      });

      await manager.createTab();

      const options = mockInitializeTabUI.mock.calls[0][2];
      options.onDraftSettingsChanged('codex', {
        model: DEFAULT_CODEX_PRIMARY_MODEL,
        effortLevel: 'high',
      });

      expect(callbacks.onTabDraftSettingsChanged).toHaveBeenCalledWith(
        'tab-1',
        'codex',
        {
          model: DEFAULT_CODEX_PRIMARY_MODEL,
    ...
    it('clamps configured max tabs above ten to ten', async () => {
      const manager = createManager({
        callbacks,
        plugin: createMockPlugin({ settings: { maxTabs: 12 } }),
      });
      ...
      expect(await manager.createTab()).toBeNull();
      expect(manager.getTabCount()).toBe(10);
    });
...
describe('TabManager - Persistence', () => {
  ...
  describe('getPersistedState', () => {
    ...
    it('should persist draft settings and orchestrator mode for blank tabs', async () => {
      const blankManager = createManager({
        tabFactory: () => createMockTabData({
          id: 'blank-codex',
          conversationId: null,
  ...
  describe('restoreState', () => {
    ...
    it('should restore draft settings and orchestrator mode for blank tabs', async () => {
      const draftSettings = {
        model: DEFAULT_CODEX_PRIMARY_MODEL,
        effortLevel: 'high',
        serviceTier: 'fast',
    ...
    it('records debug log when a persisted tab cannot be restored', async () => {
      const recordDebugLog = jest.fn();
      const plugin = createMockPlugin({ recordDebugLog });
      manager = createManager({ plugin });
      let callCount = 0;
      mockCreateTab.mockImplementation(() => {
        callCount++;
        ...
        return createMockTabData({ id: 'fallback-tab' });
      });

      await manager.restoreState({
        openTabs: [{ tabId: 'failed-tab', conversationId: 'conv-1' }],
        activeTabId: 'failed-tab',
      });
    ...
    it('records debug log when the restored active tab cannot switch conversations', async () => {
      const recordDebugLog = jest.fn();
      const plugin = createMockPlugin({
      ...
      const switchTo = jest.fn().mockRejectedValue(new Error('Switch failed'));
      manager = createManager({ plugin });
      mockCreateTab.mockImplementation((opts: any) => createMockTabData({
        id: opts.tabId,
      ...
      await manager.restoreState({
        openTabs: [{ tabId: 'restored-tab', conversationId: 'conv-1' }],
...
describe('TabManager - Provider Command Catalog', () => {
  ...
  it('prewarms an active blank provider runtime without binding the tab', async () => {
    ...
    mockInitializeTabService.mockImplementationOnce(async (targetTab: any) => {
      targetTab.service = {
      ...
      targetTab.serviceInitialized = true;
    });

    await Promise.resolve(options.onProviderChanged('opencode'));
    await flushMicrotasks();

    expect(mockInitializeTabService).toHaveBeenCalledWith(
      tab,
      plugin,
      expect.objectContaining({
        bindBlank: false,
    ...
    expect(modelSelector.updateDisplay).toHaveBeenCalled();
...
describe('TabManager - forkToNewTab at max tabs', () => {
  it('should return null when at max tabs', async () => {
    ...
    const plugin = createMockPlugin();
    // Use a small explicit limit so the fork path reaches capacity quickly.
