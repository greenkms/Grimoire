...
describe('builtInCommands', () => {
  describe('detectBuiltInCommand', () => {
    ...
    it('detects /image command with prompt arguments', () => {
      const result = detectBuiltInCommand('/image neon castle above the sea');
      expect(result).not.toBeNull();
      expect(result?.command.name).toBe('image');
      expect(result?.command.action).toBe('image');
      expect(result?.args).toBe('neon castle above the sea');
    });
  ...
  describe('BUILT_IN_COMMANDS', () => {
    ...
    it('has image command with prompt args and no provider restriction', () => {
      const imageCmd = BUILT_IN_COMMANDS.find((c) => c.name === 'image');
      expect(imageCmd).toBeDefined();
      expect(imageCmd?.action).toBe('image');
      expect(imageCmd?.hasArgs).toBe(true);
      expect(imageCmd?.requiredCapability).toBeUndefined();
    });
  ...
  describe('getBuiltInCommandsForDropdown - provider filtering', () => {
    ...
    it('returns only commands supported by codex capabilities', () => {
      ...
      expect(commands.map(c => c.name)).toEqual(['clear', 'add-dir', 'resume', 'fork', 'image']);
    });
  });
