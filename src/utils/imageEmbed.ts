/**
 * Grimoire - Image Embed Utilities
 *
...
function createImageHtml(
  ...
  return `<span class="grimoire-embedded-image"><img src="${escapeHtml(src)}" alt="${alt}" loading="lazy"${style}></span>`;
}

function createFallbackHtml(wikilink: string): string {
  return `<span class="grimoire-embedded-image-fallback">${escapeHtml(wikilink)}</span>`;
}
