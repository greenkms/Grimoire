...
function createMockIndicator() {
  ...
  indicator.addClass('grimoire-selection-indicator');
  indicator.addClass('grimoire-hidden');
  return indicator;
}
...
function createMockContextRow() {
  const elements: Record<string, any> = {
    '.grimoire-selection-indicator': createMockIndicator(),
    '.grimoire-canvas-indicator': createMockEl(),
    '.grimoire-file-indicator': null,
    '.grimoire-image-preview': null,
  };
  elements['.grimoire-canvas-indicator'].addClass('grimoire-canvas-indicator');
  elements['.grimoire-canvas-indicator'].addClass('grimoire-hidden');
...
describe('SelectionController', () => {
  ...
  it('preserves selection when active leaf is the Grimoire view in a detached window or own tab', () => {
    app.workspace.activeLeaf = { view: { getViewType: () => 'grimoire-view' } };
  ...
  it('clears selection when active leaf is a non-Grimoire, non-Markdown view and chat is not focused', () => {
    app.workspace.activeLeaf = { view: { getViewType: () => 'settings' } };
    controller = new SelectionController(
      app,
      indicatorEl,
      inputEl,
  ...
  describe('Reading mode (preview)', () => {
    ...
    it('clears CSS highlight when reading mode selection is deselected', () => {
      ...
      expect(mockHighlights.delete).toHaveBeenCalledWith('grimoire-selection');
    });
  ...
  it('keeps context row visible when canvas selection indicator is visible', () => {
    ...
    canvasIndicator.addClass('grimoire-canvas-indicator');
    contextRowEl.querySelector.mockImplementation((selector: string) => {
      if (selector === '.grimoire-canvas-indicator') return canvasIndicator;
      return null;
    });
