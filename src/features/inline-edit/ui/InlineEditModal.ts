...
export function buildInlineEditInputDecorations(options: {
  ...
  return Decoration.set([
    Decoration.line({
      class: 'grimoire-inline-input-line',
    }).range(lineStart),
    Decoration.widget({
...
function appendDiffOps(container: HTMLElement, ops: DiffOp[]): void {
  for (const op of ops) {
    switch (op.type) {
      case 'delete':
        container.createSpan({ cls: 'grimoire-diff-del', text: op.text });
        break;
      case 'insert':
        container.createSpan({ cls: 'grimoire-diff-ins', text: op.text });
        break;
      default:
...
class InlineEditController {
  ...
  private async generate() {
    ...
    this.spinnerEl.removeClass('grimoire-hidden');
  ...
  private showAgentReply(message: string) {
    ...
    this.agentReplyEl.removeClass('grimoire-hidden');
