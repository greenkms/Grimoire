/**
 * Grimoire - Resume session dropdown
 *
...
export class ResumeSessionDropdown {
  ...
  constructor(
    ...
    this.dropdownEl = this.containerEl.createDiv({ cls: 'grimoire-resume-dropdown' });
    this.render();
  ...
  private updateSelection(): void {
    const items = this.dropdownEl.querySelectorAll('.grimoire-resume-item');
  ...
  private render(): void {
    ...
    const list = this.dropdownEl.createDiv({ cls: 'grimoire-resume-list' });

    for (let i = 0; i < this.conversations.length; i++) {
      const conv = this.conversations[i];
      const isCurrent = conv.id === this.currentConversationId;

      const item = list.createDiv({ cls: 'grimoire-resume-item' });
