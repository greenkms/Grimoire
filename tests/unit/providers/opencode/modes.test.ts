...
describe('OpenCode mode settings', () => {
  ...
  it('normalizes saved custom mode selections back to the managed full-access mode', () => {
    expect(normalizeManagedOpencodeSelectedMode('compaction')).toBe(OPENCODE_FULL_ACCESS_MODE_ID);
  });

  it('normalizes the legacy build id back to the managed full-access mode', () => {
    expect(normalizeManagedOpencodeSelectedMode(OPENCODE_BUILD_MODE_ID)).toBe(OPENCODE_FULL_ACCESS_MODE_ID);
  });

  it('normalizes the legacy yolo mode id back to the managed full-access mode', () => {
    expect(normalizeManagedOpencodeSelectedMode(OPENCODE_LEGACY_YOLO_MODE_ID)).toBe(OPENCODE_FULL_ACCESS_MODE_ID);
  });

  it('maps shared permission modes onto managed OpenCode modes', () => {
    expect(resolveOpencodeModeForPermissionMode('full_access')).toBe(OPENCODE_FULL_ACCESS_MODE_ID);
  ...
  it('maps managed OpenCode modes back to shared permission modes', () => {
    expect(resolvePermissionModeForManagedOpencodeMode(OPENCODE_BUILD_MODE_ID)).toBe('full_access');
    expect(resolvePermissionModeForManagedOpencodeMode(OPENCODE_FULL_ACCESS_MODE_ID)).toBe('full_access');
    expect(resolvePermissionModeForManagedOpencodeMode(OPENCODE_LEGACY_YOLO_MODE_ID)).toBe('full_access');
...
describe('opencodeChatUIConfig permission mode wiring', () => {
  ...
  it('maps shared permission mode changes back into managed OpenCode modes', () => {
    ...
    opencodeChatUIConfig.applyPermissionMode?.('full_access', settings);
    expect((settings.providerConfigs as Record<string, Record<string, unknown>>).opencode.selectedMode).toBe(OPENCODE_FULL_ACCESS_MODE_ID);
  });
});
