...
describe('textareaResize', () => {
  ...
  it('measures from base height so previously grown content can shrink', () => {
    ...
    textarea.style.setProperty('--grimoire-textarea-min-height', '116px');

    autoResizeTextarea(textarea);

    expect(textarea.style.getPropertyValue('--grimoire-textarea-min-height')).toBe('60px');
  });
  ...
  it('measures from base height so long content does not bounce', () => {
    ...
    expect(textarea.style.getPropertyValue('--grimoire-textarea-min-height')).toBe('116px');
  });
...
function createResizeTextarea({
  ...
  const isBaseHeight = () =>
    textarea.style.getPropertyValue('--grimoire-textarea-min-height') === `${TEXTAREA_BASE_MIN_HEIGHT}px`;
