...
describe('ClaudeChatRuntime', () => {
  let mockPlugin: Partial<GrimoirePlugin>;
  ...
  beforeEach(() => {
    ...
    mockPlugin = {
      ...
      settings: {
        ...
        providerConfigs: {
          claude: { enabled: true, loadUserSettings: false },
        },
        model: 'claude-3-5-sonnet',
        permissionMode: 'ask' as const,
        thinkingBudget: 0,
        mediaFolder: 'grimoire-media',
        systemPrompt: '',
    ...
    service = new ClaudeChatRuntime(mockPlugin as GrimoirePlugin, mockMcpManager);
  });
  ...
  describe('Approval Callback', () => {
    ...
    describe('createApprovalCallback permission flow', () => {
      ...
      it('auto-allows trusted Obsidian MCP read tools in safe mode', async () => {
        ...
        const result = await canUseTool(
          'mcp__mcp_obsidian__obsidian_get_file_contents',
          { filepath: 'Marine Life/Осьминоги (Octopuses).md' },
          canUseToolOptions,
        );

        expect(result).toEqual({ behavior: 'allow', updatedInput: { filepath: 'Marine Life/Осьминоги (Octopuses).md' } });
        expect(callback).not.toHaveBeenCalled();
      });

      it('still asks for trusted Obsidian MCP write tools in safe mode', async () => {
        (mockPlugin as any).settings.permissionMode = 'normal';
        const callback = jest.fn().mockResolvedValue('allow');
        service.setApprovalCallback(callback);
      ...
      it('still asks for unknown MCP tools even when their name looks read-only', async () => {
        (mockPlugin as any).settings.permissionMode = 'normal';
        const callback = jest.fn().mockResolvedValue('allow');
        service.setApprovalCallback(callback);
  ...
  describe('applyDynamicUpdates', () => {
    ...
    it('should update permission mode when changed', async () => {
      (mockPlugin as any).settings.permissionMode = 'full_access';
  ...
  describe('query() method', () => {
    ...
    it('logs safe Claude SDK plan usage metadata for debugging', async () => {
      sdkMock.setMockMessages([
        {
      ...
      expect(mockPlugin.recordDebugLog).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('logs reset-only Claude SDK rate limit metadata as quota usage', async () => {
      sdkMock.setMockMessages([
        {
          type: 'rate_limit_event',
          rate_limit_info: {
      ...
      expect(mockPlugin.recordDebugLog).toHaveBeenCalledWith(expect.objectContaining({
        data: expect.objectContaining({
          ...
          messageType: 'rate_limit_event',
          providerId: 'claude',
          rateLimitInfoFields: 'rateLimitType,resetsAt,status',
          rateLimitType: 'five_hour',
          reason: 'accepted_reset_only',
          status: 'allowed',
          usageKind: 'quota',
        }),
        event: 'sdk.usage_message',
    ...
    it('logs Claude SDK token cost metadata as spend usage', async () => {
      sdkMock.setMockMessages([
        {
          type: 'result',
          subtype: 'success',
