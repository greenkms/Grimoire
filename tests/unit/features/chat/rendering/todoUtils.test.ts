...
describe('todoUtils', () => {
  ...
  describe('renderTodoItems', () => {
    it('should render todo items with status icons and text', () => {
      ...
      expect(container._children[0].hasClass('grimoire-todo-completed')).toBe(true);
      expect(setIcon).toHaveBeenCalledWith(expect.anything(), 'check');

      // Second item: in_progress shows activeForm
      expect(container._children[1].hasClass('grimoire-todo-in_progress')).toBe(true);

      // Third item: pending
      expect(container._children[2].hasClass('grimoire-todo-pending')).toBe(true);
    });
