...
describe('McpStorage', () => {
  describe('load', () => {
    ...
    it('loads servers with disabledTools from _grimoire metadata', async () => {
      const config = {
        mcpServers: {
    ...
    it('applies defaults when no _grimoire metadata exists', async () => {
      const config = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
    ...
    it('loads description from _grimoire metadata', async () => {
      const config = {
  ...
  describe('save', () => {
    it('saves disabledTools to _grimoire metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      expect(saved._grimoire.servers.alpha.disabledTools).toEqual(['tool_a', 'tool_b']);
    });
    ...
    it('omits disabledTools from metadata when empty', async () => {
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      // No _grimoire since all fields are default
      expect(saved._grimoire).toBeUndefined();
    });

    it('preserves existing _grimoire metadata when saving', async () => {
      const existing = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._grimoire.customField).toBe('should be preserved');
      expect(saved._grimoire.servers.alpha.disabledTools).toEqual(['tool_a']);
    });
    ...
    it('saves description to _grimoire metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._grimoire.servers.alpha.description).toBe('A test server');
    });

    it('stores enabled=false in _grimoire when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._grimoire.servers.alpha.enabled).toBe(false);
    });

    it('stores contextSaving=false in _grimoire when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._grimoire.servers.alpha.contextSaving).toBe(false);
    });

    it('removes _grimoire.servers when all metadata is default', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _grimoire: { servers: { alpha: { enabled: false } } },
      };
      const adapter = createMockAdapter({
        '.claude/mcp.json': JSON.stringify(existing),
      });
    ...
    it('preserves non-servers _grimoire fields when removing servers', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _grimoire: {
          customField: 'keep',
      ...
      const saved = JSON.parse(adapter._store['.claude/mcp.json']);
      expect(saved._grimoire).toEqual({ customField: 'keep' });
    });
