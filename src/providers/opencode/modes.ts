...
export const OPENCODE_FULL_ACCESS_MODE_ID = 'grimoire-full-access';
export const OPENCODE_LEGACY_YOLO_MODE_ID = 'grimoire-yolo';
export const OPENCODE_SAFE_MODE_ID = 'grimoire-safe';
...
export function normalizeManagedOpencodeSelectedMode(
  ...
  const canonicalModeId = normalized === OPENCODE_BUILD_MODE_ID
    || normalized === OPENCODE_LEGACY_YOLO_MODE_ID
...
export function resolveOpencodeModeForPermissionMode(
  ...
  if (coercePermissionMode(permissionMode) === 'full_access' && managedModeIds.has(OPENCODE_FULL_ACCESS_MODE_ID)) {
    return OPENCODE_FULL_ACCESS_MODE_ID;
  }
  if (managedModeIds.has(OPENCODE_FULL_ACCESS_MODE_ID)) {
    return OPENCODE_FULL_ACCESS_MODE_ID;
  }
...
export function resolvePermissionModeForManagedOpencodeMode(
  ...
  if (
    ...
    || modeId === OPENCODE_FULL_ACCESS_MODE_ID
    || modeId === OPENCODE_LEGACY_YOLO_MODE_ID
  ) {
    return 'full_access';
  }
