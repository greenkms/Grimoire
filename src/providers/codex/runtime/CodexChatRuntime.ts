...
export class CodexChatRuntime implements ChatRuntime {
  ...
  async *query(
    ...
    try {
      ...
      let sentBaseInstructionsThisQuery = false;
      ...
      if (this.pendingFork) {
        ...
        sentBaseInstructionsThisQuery = true;
  ...
  private async startAppServer(launchSpec: CodexLaunchSpec, clientConfigKey: string): Promise<void> {
    ...
    this.wirePlanUsageHandler();
    codexPlanUsageStore.setRateLimitsReader(async () => {
      if (!this.transport) {
        return null;
      }
      return await this.transport.request('account/rateLimits/read', {});
    });
  ...
  private async refreshModelDiscovery(): Promise<void> {
    if (!this.transport) {
      return;
    }

    try {
      const models = await listCodexModelsViaTransport(this.transport);
      if (models.length === 0) {
        return;
      }
      ...
      // Older Codex CLI builds do not expose model/list; keep static fallback models.
    }
  }
  ...
  private wirePlanUsageHandler(): void {
    this.transport?.onNotification('account/rateLimits/updated', (params) => {
      const changed = codexPlanUsageStore.updateFromRateLimits(params);
      this.plugin.recordDebugLog?.({
        data: {
          changed,
          providerId: this.providerId,
        },
        event: 'rateLimits.notification',
  ...
  private refreshPlanUsageFromServerInBackground(): void {
    this.plugin.recordDebugLog?.({
      ...
      event: 'rateLimits.refreshQueued',
      level: 'debug',
      scope: 'codex.usage',
    });
    void codexPlanUsageStore.refreshUsage({
      plugin: this.plugin,
      providerId: this.providerId,
  ...
  private async shutdownProcess(): Promise<void> {
    codexPlanUsageStore.setRateLimitsReader(null);
    if (this.transport) {
  ...
  private resolveOrchestratorMode(
    request: ChatTurnRequest,
    queryOptions?: ChatRuntimeQueryOptions,
  ): boolean {
    return request.orchestratorMode === true || queryOptions?.orchestratorMode === true;
  }
