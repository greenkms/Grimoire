...
function formatClaudeModelFallbackLabel(model: string): string {
  return model
    .trim()
    .replace(/^claude[-_/]/i, '')
    .replace(/-(\d+)-(\d+)/g, ' $1.$2')
...
function buildClaudeResponseMetadata(model: unknown): AssistantResponseMetadata | undefined {
  ...
  return {
    ...
    model: trimmed,
    modelLabel: DEFAULT_CLAUDE_MODELS.find(option => option.value === trimmed)?.label
      ?? formatClaudeModelFallbackLabel(trimmed),
  };
}
...
export function parseSDKMessageToChat(
  ...
  const responseMetadata = sdkMsg.type === 'assistant'
    ? buildClaudeResponseMetadata(sdkMsg.message?.model)
    : undefined;
