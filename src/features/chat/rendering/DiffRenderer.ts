...
export function renderDiffContent(
  ...
  if (allInserts && diffLines.length > NEW_FILE_DISPLAY_CAP) {
    const hunkEl = containerEl.createDiv({ cls: 'grimoire-diff-hunk' });
    for (const line of diffLines.slice(0, NEW_FILE_DISPLAY_CAP)) {
      const lineEl = hunkEl.createDiv({ cls: 'grimoire-diff-line grimoire-diff-insert' });
      const prefixEl = lineEl.createSpan({ cls: 'grimoire-diff-prefix' });
      prefixEl.setText('+');
      const contentEl = lineEl.createSpan({ cls: 'grimoire-diff-text' });
      contentEl.setText(line.text || ' ');
    }
    const remaining = diffLines.length - NEW_FILE_DISPLAY_CAP;
    const separator = containerEl.createDiv({ cls: 'grimoire-diff-separator' });
  ...
  if (hunks.length === 0) {
    // No changes
    const noChanges = containerEl.createDiv({ cls: 'grimoire-diff-no-changes' });
    noChanges.setText(t('chat.ui.tools.noChanges'));
    return;
  }
  ...
  hunks.forEach((hunk, hunkIndex) => {
    ...
    for (const line of hunk.lines) {
      const lineEl = hunkEl.createDiv({ cls: `grimoire-diff-line grimoire-diff-${line.type}` });
