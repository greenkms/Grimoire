import type { GrokDiscoveredModel, GrokThinkingOptionsByModel } from '../models';
import type { GrokMode } from '../modes';
...
export function sameModes(left: GrokMode[], right: GrokMode[]): boolean {
  if (left.length !== right.length) {
    return false;
  }
...
export function sameDiscoveredModels(
  left: GrokDiscoveredModel[],
  right: GrokDiscoveredModel[],
): boolean {
  if (left.length !== right.length) {
    return false;
  }
...
export function sameThinkingOptionsByModel(
  left: GrokThinkingOptionsByModel,
  right: GrokThinkingOptionsByModel,
): boolean {
  const leftEntries = Object.entries(left);
  if (leftEntries.length !== Object.keys(right).length) {
    return false;
