...
describe('ToolCallRenderer', () => {
  ...
  describe('renderToolCall', () => {
    it('renders Bash as a compact activity-trace step with expandable command input', () => {
      ...
      expect(toolEl.hasClass('grimoire-tool-step')).toBe(true);
  ...
  describe('renderStoredToolCall', () => {
    it('renders completed Glob with a result label and trace body sections', () => {
      const parentEl = createMockEl();
      const toolCall = createToolCall({
        name: 'Glob',
        status: 'completed',
      ...
      expect(toolEl.hasClass('grimoire-tool-step')).toBe(true);
      expect(toolEl.querySelector('.grimoire-tool-name')?.textContent).toBe('Globbed files');
      expect(toolEl.querySelector('.grimoire-tool-summary')?.textContent).toBe('**/*.md');
      expect(toolEl.querySelector('.grimoire-tool-result')?.textContent).toBe('3 files');
      expect(toolEl.querySelector('.grimoire-tool-io-caption')?.textContent).toBe('MATCHED');
    });

    it('previews long Bash output and reveals all lines on demand', () => {
      const parentEl = createMockEl();
      ...
      expect(toolEl.querySelector('.grimoire-tool-io-code')?.textContent).toBe('$ npm test');
      expect(toolEl.querySelectorAll('.grimoire-tool-line')).toHaveLength(20);
      expect(toolEl.querySelector('.grimoire-tool-truncated')?.textContent).toContain('5 more lines');

      const showAllButton = toolEl.querySelector('.grimoire-tool-show-all') as HTMLElement;
      expect(showAllButton).not.toBeNull();
      expect(showAllButton.textContent).toContain('Show all');

      showAllButton.click();

      const lines = Array.from(toolEl.querySelectorAll('.grimoire-tool-line')).map(line => line.textContent);
      expect(lines).toHaveLength(25);
      expect(lines.at(-1)).toBe('line 25');
      expect(toolEl.querySelector('.grimoire-tool-show-all')?.textContent).toContain('Show preview');
    });
  ...
  describe('renderStoredToolCallGroup', () => {
    it('renders consecutive vault searches as an expandable grouped trace', () => {
      const parentEl = createMockEl();
      const calls = [
        createToolCall({
          id: 'grep-1',
      ...
      const groupEl = renderStoredToolCallGroup(parentEl, calls);
      ...
      const subEl = groupEl.querySelector('.grimoire-tool-group-sub') as HTMLElement;
      expect(subEl.hasClass('grimoire-hidden')).toBe(true);

      const headerEl = groupEl.querySelector('.grimoire-tool-group-header') as HTMLElement;
      headerEl.click();

      expect(groupEl.hasClass('expanded')).toBe(true);
      expect(subEl.hasClass('grimoire-hidden')).toBe(false);
    });
  });
  ...
  describe('updateToolCallResult', () => {
    it('should update status indicator', () => {
      ...
      const statusEl = toolEl.querySelector('.grimoire-tool-status');
  ...
  describe('apply_patch expanded rendering', () => {
    it('renders parsed patch diffs from tool input', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const headers = Array.from(toolEl.querySelectorAll('.grimoire-tool-patch-header')).map(el => el.textContent);
      const statusEl = toolEl.querySelector('.grimoire-tool-status');
      const diffTexts = Array.from(toolEl.querySelectorAll('.grimoire-diff-text')).map(el => el.textContent);

      expect(headers).toHaveLength(0);
      expect(statusEl?.hasClass('grimoire-write-edit-stats')).toBe(true);
    ...
    it('keeps the error status icon for failed apply_patch calls', () => {
      ...
      expect(statusEl?.hasClass('grimoire-write-edit-stats')).toBe(false);
      expect(setIcon).toHaveBeenCalledWith(expect.anything(), 'x');
    });
    ...
    it('falls back to file changes when patch text is unavailable', () => {
      ...
      const toolEl = renderStoredToolCall(parentEl, toolCall);
      const lines = Array.from(toolEl.querySelectorAll('.grimoire-tool-line')).map(el => el.textContent);
  ...
  describe('updateToolCallResult for TodoWrite', () => {
    it('should update todo status and content', () => {
      ...
      const statusEl = parentEl.querySelector('.grimoire-tool-status');
      expect(statusEl?.hasClass('status-completed')).toBe(true);
    });
