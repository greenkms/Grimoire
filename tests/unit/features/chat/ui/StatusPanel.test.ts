...
class MockElement {
  ...
  private syncDisplay(): void {
    if (this.classList.has('grimoire-hidden')) {
      this.style.display = 'none';
      return;
    }
    if (
      this.classList.has('grimoire-status-panel-todos')
      || this.classList.has('grimoire-status-panel-content')
      || this.classList.has('grimoire-status-panel-bash')
      || this.classList.has('grimoire-status-panel-bash-content')
      || this.classList.has('grimoire-tool-content')
    ) {
  ...
  querySelectorAll(selector: string): MockElement[] {
    ...
    const match = (el: MockElement): boolean => {
      ...
      // Handle class selectors like .grimoire-status-panel
...
describe('StatusPanel', () => {
  ...
  describe('mount', () => {
    it('should create panel element when mounted', () => {
      ...
      expect(containerEl.querySelector('.grimoire-status-panel')).not.toBeNull();
    });

    it('should hide the panel shell initially', () => {
      panel.mount(containerEl as unknown as HTMLElement);

      const panelEl = containerEl.querySelector('.grimoire-status-panel');
      expect(panelEl).not.toBeNull();
      expect(panelEl!.hasClass('grimoire-hidden')).toBe(true);
    });

    it('should create hidden todo container initially', () => {
      panel.mount(containerEl as unknown as HTMLElement);

      const todoContainer = containerEl.querySelector('.grimoire-status-panel-todos');
  ...
  describe('updateTodos', () => {
    ...
    it('should show panel when todos are provided', () => {
      ...
      const panelEl = containerEl.querySelector('.grimoire-status-panel');
      const todoContainer = containerEl.querySelector('.grimoire-status-panel-todos');
      expect(panelEl!.hasClass('grimoire-hidden')).toBe(false);
    ...
    it('should display correct task count', () => {
      ...
      const label = containerEl.querySelector('.grimoire-status-panel-label');
    ...
    it('should show current task in collapsed header', () => {
      ...
      const current = containerEl.querySelector('.grimoire-status-panel-current');
    ...
    it('should render all todo items in content area', () => {
      ...
      const items = containerEl.querySelectorAll('.grimoire-todo-item');
    ...
    it('should apply correct status classes to items', () => {
      ...
      expect(containerEl.querySelector('.grimoire-todo-pending')).not.toBeNull();
      expect(containerEl.querySelector('.grimoire-todo-in_progress')).not.toBeNull();
      expect(containerEl.querySelector('.grimoire-todo-completed')).not.toBeNull();
    });
  ...
  describe('toggle', () => {
    ...
    it('should expand content on header click', () => {
      const header = containerEl.querySelector('.grimoire-status-panel-header');
      const content = containerEl.querySelector('.grimoire-status-panel-content');
    ...
    it('should show list icon in header', () => {
      const icon = containerEl.querySelector('.grimoire-status-panel-icon');
    ...
    it('should hide current task when expanded', () => {
      ...
      expect(containerEl.querySelector('.grimoire-status-panel-current')).not.toBeNull();

      header!.click();

      expect(containerEl.querySelector('.grimoire-status-panel-current')).toBeNull();
    });
  ...
  describe('accessibility', () => {
    ...
    it('should hide status icons from screen readers', () => {
      ...
      const icon = containerEl.querySelector('.grimoire-todo-status-icon');
  ...
  describe('destroy', () => {
    it('should remove panel from DOM', () => {
      ...
      expect(containerEl.querySelector('.grimoire-status-panel')).toBeNull();
    });
  ...
  describe('bash outputs', () => {
    ...
    it('should render bash section with header and entries', () => {
      ...
      const panelEl = containerEl.querySelector('.grimoire-status-panel');
      const bashContainer = containerEl.querySelector('.grimoire-status-panel-bash');
      expect(panelEl!.hasClass('grimoire-hidden')).toBe(false);
      expect(bashContainer).not.toBeNull();
      expect(bashContainer!.style.display).toBe('block');

      const header = containerEl.querySelector('.grimoire-status-panel-bash-header');
      expect(header).not.toBeNull();
      const label = header!.querySelector('.grimoire-tool-label');
      expect(label).not.toBeNull();
      expect(label!.textContent).toBe('Command panel');

      const entries = containerEl.querySelectorAll('.grimoire-status-panel-bash-entry');
    ...
    it('should collapse and expand the bash section', () => {
      ...
      const content = containerEl.querySelector('.grimoire-status-panel-bash-content');
      ...
      header!.click();
      expect(content!.style.display).toBe('none');
      const collapsedHeader = containerEl.querySelector('.grimoire-status-panel-bash-header');
      expect(collapsedHeader).not.toBeNull();
      const collapsedLabel = collapsedHeader!.querySelector('.grimoire-tool-label');
      expect(collapsedLabel).not.toBeNull();
      expect(collapsedLabel!.textContent).toBe('echo hello');

      header!.click();
      expect(content!.style.display).toBe('block');
      const expandedHeaderAgain = containerEl.querySelector('.grimoire-status-panel-bash-header');
      expect(expandedHeaderAgain).not.toBeNull();
      const expandedLabelAgain = expandedHeaderAgain!.querySelector('.grimoire-tool-label');
    ...
    it('should collapse and expand individual bash output entries', () => {
      ...
      const entry = containerEl.querySelector('.grimoire-status-panel-bash-entry');
      expect(entry).not.toBeNull();

      const entryHeader = entry!.querySelector('.grimoire-tool-header');
      const entryContent = entry!.querySelector('.grimoire-tool-content');
      ...
      const entryAfterClick = containerEl.querySelector('.grimoire-status-panel-bash-entry');
      const contentAfterClick = entryAfterClick!.querySelector('.grimoire-tool-content');
      const headerAfterClick = entryAfterClick!.querySelector('.grimoire-tool-header');
      ...
      const entryAfterKeydown = containerEl.querySelector('.grimoire-status-panel-bash-entry');
      const contentAfterKeydown = entryAfterKeydown!.querySelector('.grimoire-tool-content');
      const headerAfterKeydown = entryAfterKeydown!.querySelector('.grimoire-tool-header');
    ...
    it('should clear bash outputs via action button', () => {
      ...
      const clearButton = containerEl.querySelector('.grimoire-status-panel-bash-action-clear');
    ...
    it('should copy latest bash output via action button', async () => {
      ...
      const copyButton = containerEl.querySelector('.grimoire-status-panel-bash-action-copy');
    ...
    it('should update a running bash output to completed with output', () => {
      ...
      let entry = containerEl.querySelector('.grimoire-status-panel-bash-entry');
      let text = entry!.querySelector('.grimoire-tool-result-text');
      expect(text!.textContent).toBe('Running...');

      panel.updateBashOutput('bash-1', { status: 'completed', output: 'hello', exitCode: 0 });

      entry = containerEl.querySelector('.grimoire-status-panel-bash-entry');
      text = entry!.querySelector('.grimoire-tool-result-text');
      expect(text!.textContent).toBe('hello');

      const statusEl = entry!.querySelector('.grimoire-tool-status');
    ...
    it('should update a running bash output to error', () => {
      ...
      const entry = containerEl.querySelector('.grimoire-status-panel-bash-entry');
      const text = entry!.querySelector('.grimoire-tool-result-text');
    ...
    it('should set aria-expanded on the bash section header', () => {
      ...
      header!.click();
      const headerAfterCollapse = containerEl.querySelector('.grimoire-status-panel-bash-header');
      expect(headerAfterCollapse!.getAttribute('aria-expanded')).toBe('false');

      headerAfterCollapse!.click();
      const headerAfterExpand = containerEl.querySelector('.grimoire-status-panel-bash-header');
