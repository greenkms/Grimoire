...
| 제공자 슬래시 명령 | 예 | 아니요 | 예 | 예 | 예 | 예 | 아니요 | 아니요 | 예 |
| Grimoire 관리 MCP UI | 예 | 아니요 | 아니요 | 아니요 | 아니요 | 아니요 | 아니요 | 아니요 | 아니요 |
...
Qwen은 자격 증명과 구성을 `~/.qwen/settings.json`에서 관리합니다. CLI 또는 Qwen 소유의 `.env`와 환경 변수를 우선 사용하세요. Grimoire는 제공자 환경 변수를 전달할 수 있지만 Qwen 자격 증명이나 MCP 구성을 관리하지 않습니다. Qwen이 ACP 토큰 또는 비용 메타데이터를 내보낼 때만 사용량이 표시됩니다. Qwen은 현재 Grimoire의 포크나 되돌리기 제어를 지원하지 않습니다.
