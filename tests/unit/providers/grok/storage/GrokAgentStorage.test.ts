...
import {
  createGrokAgentPersistenceKey,
  GROK_AGENT_PATH,
  GROK_AGENTS_PATH,
  GrokAgentStorage,
  parseGrokAgentMarkdown,
  parseGrokAgentPersistenceKey,
  serializeGrokAgentMarkdown,
} from '@/providers/grok/storage/GrokAgentStorage';
...
describe('parseGrokAgentMarkdown', () => {
  it('parses required fields and derives the name from the file path', () => {
    const result = parseGrokAgentMarkdown(BASIC_MARKDOWN, '.grok/agent/review.md');
    ...
    expect(result!.persistenceKey).toBe(
      createGrokAgentPersistenceKey({ filePath: '.grok/agent/review.md' }),
    );
  });

  it('supports nested agent paths and maxSteps fallback', () => {
    const result = parseGrokAgentMarkdown(`---
description: "Explores bugs."
maxSteps: 7
---
Trace the bug carefully.
`, '.grok/agents/nested/explorer.md');

    expect(result).not.toBeNull();
    expect(result!.name).toBe('nested/explorer');
    expect(result!.steps).toBe(7);
    expect(result!.persistenceKey).toBe(
      createGrokAgentPersistenceKey({ filePath: '.grok/agents/nested/explorer.md' }),
    );
  });

  it('preserves supported optional fields and unknown frontmatter', () => {
    const result = parseGrokAgentMarkdown(FULL_MARKDOWN, '.grok/agents/reviewer.md');
  ...
  it('returns null when required fields are missing', () => {
    expect(parseGrokAgentMarkdown('---\nmode: subagent\n---\nPrompt', 'review.md')).toBeNull();
  });
});

describe('serializeGrokAgentMarkdown', () => {
  it('serializes supported fields and round-trips through parse', () => {
    const original: GrokAgentDefinition = {
    ...
    const markdown = serializeGrokAgentMarkdown(original);
    const reparsed = parseGrokAgentMarkdown(markdown, '.grok/agents/nested/reviewer.md');
...
describe('GrokAgentStorage', () => {
  describe('loadAll', () => {
    it('loads markdown agents recursively from both supported Grok Build roots', async () => {
      const adapter = createMockAdapter({
        '.grok/agent/review.md': BASIC_MARKDOWN,
        '.grok/agents/nested/explorer.md': `---
description: "Explores bugs."
mode: all
---
Trace the bug carefully.
`,
        '.grok/agent/README.txt': 'ignore me',
      });

      const storage = new GrokAgentStorage(adapter);
      const agents = await storage.loadAll();

      expect(agents.map((agent) => agent.name).sort()).toEqual(['nested/explorer', 'review']);
      expect(adapter.listFilesRecursive).toHaveBeenCalledWith(GROK_AGENTS_PATH);
      expect(adapter.listFilesRecursive).toHaveBeenCalledWith(GROK_AGENT_PATH);
    });

    it('prefers the singular root when duplicate agent names exist in both roots', async () => {
      const adapter = createMockAdapter({
        '.grok/agents/review.md': `---
description: "Plural root"
mode: subagent
---
Plural prompt.
`,
        '.grok/agent/review.md': `---
...
`,
      ...
      const agent = await storage.load({
        ...
        description: '',
        prompt: '',
        persistenceKey: createGrokAgentPersistenceKey({ filePath: '.grok/agents/nested/explorer.md' }),
      });
  ...
  describe('save', () => {
    it('writes new nested agents to the singular Grok Build root', async () => {
      const adapter = createMockAdapter({});
      const storage = new GrokAgentStorage(adapter);
      ...
      expect(adapter.ensureFolder).toHaveBeenCalledWith('.grok/agent/nested');
      expect(adapter.write).toHaveBeenCalledWith(
        '.grok/agent/nested/reviewer.md',
    ...
    it('preserves the existing plural-root backing file when the name is unchanged', async () => {
      ...
      await storage.save({
        ...
        description: 'Reviews code.',
        prompt: 'Review carefully.',
        persistenceKey: createGrokAgentPersistenceKey({ filePath: '.grok/agents/legacy/reviewer.md' }),
      });

      expect(adapter.write).toHaveBeenCalledWith(
        '.grok/agents/legacy/reviewer.md',
        expect.any(String),
      );
    ...
    it('preserves the existing singular-root backing file when the name is unchanged', async () => {
      ...
      await storage.save({
        ...
        description: 'Reviews code.',
        prompt: 'Review carefully.',
        persistenceKey: createGrokAgentPersistenceKey({ filePath: '.grok/agent/legacy/reviewer.md' }),
      });

      expect(adapter.write).toHaveBeenCalledWith(
        '.grok/agent/legacy/reviewer.md',
        expect.any(String),
      );
      expect(adapter.delete).not.toHaveBeenCalled();
    });
    ...
    it('renames the backing file when the agent name changes', async () => {
      ...
      expect(adapter.write).toHaveBeenCalledWith(
        '.grok/agent/reviewer.md',
        expect.any(String),
      );
      expect(adapter.delete).toHaveBeenCalledWith('.grok/agent/legacy/reviewer.md');
    });
  });
  ...
  describe('delete', () => {
    it('deletes the resolved nested file', async () => {
      ...
      await storage.delete({
        ...
        description: '',
        prompt: '',
        persistenceKey: createGrokAgentPersistenceKey({ filePath: '.grok/agent/nested/reviewer.md' }),
      });

      expect(adapter.delete).toHaveBeenCalledWith('.grok/agent/nested/reviewer.md');
    });
  });
});

describe('Grok agent persistence keys', () => {
  it('round-trips nested file identity for the singular Grok Build root', () => {
    const key = createGrokAgentPersistenceKey({ filePath: '.grok/agent/nested/reviewer.md' });

    expect(parseGrokAgentPersistenceKey(key)).toEqual({ filePath: '.grok/agent/nested/reviewer.md' });
  });

  it('parses direct singular-root storage paths for backward compatibility', () => {
    expect(parseGrokAgentPersistenceKey('.grok/agent/nested/reviewer.md')).toEqual({
      filePath: '.grok/agent/nested/reviewer.md',
    });
  });

  it('parses direct plural-root storage paths for backward compatibility', () => {
    expect(parseGrokAgentPersistenceKey('.grok/agents/nested/reviewer.md')).toEqual({
      filePath: '.grok/agents/nested/reviewer.md',
    });
  });

  it('parses legacy encoded relative paths produced by the initial implementation', () => {
    expect(parseGrokAgentPersistenceKey('grok-agent:nested%2Freviewer.md')).toEqual({
      filePath: '.grok/agents/nested/reviewer.md',
    });
  });
});
