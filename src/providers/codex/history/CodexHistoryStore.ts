  ...
  switch (item.type) {
    case 'agent_message':
      if (eventType === 'item.completed' || eventType === 'item.updated') {
        const text = sanitizeCodexAssistantText(item.text ?? '');
        if (text) {
  ...
  switch (payload.type) {
    case 'message': {
      ...
      if (messagePayload.role === 'user') {
        ...
        const turn = ensureTurn(ctx.turns, ctx.turnOrder, nextTurnId(ctx), ctx.currentTurnId, timestamp);
        const bubble = ensureAssistantBubble(turn, timestamp);
        const visibleText = sanitizeCodexAssistantText(text);
        if (visibleText) {
          appendUniqueChunk(bubble.contentChunks, visibleText);
        }
      }
      break;
    }
  ...
  switch (payload.type) {
    ...
    case 'agent_message': {
      ...
      if (typeof msg === 'string') {
        appendUniqueChunk(bubble.contentChunks, sanitizeCodexAssistantText(msg));
      }
      break;
    }
  ...
  switch (item.type) {
    case 'agent_message': {
      if ((eventType === 'item.updated' || eventType === 'item.completed') && item.text) {
        const turn = ensureTurn(ctx.turns, ctx.turnOrder, nextTurnId(ctx), ctx.currentTurnId, timestamp);
        const bubble = ensureAssistantBubble(turn, timestamp);
        replaceLatestChunk(bubble.contentChunks, sanitizeCodexAssistantText(item.text));
      }
      break;
    }
