...
function getRenderedItems(containerEl: any): { title: string; isCurrent: boolean }[] {
  const dropdownEl = containerEl.children.find(
    (c: any) => c.hasClass('grimoire-resume-dropdown')
  );
  if (!dropdownEl) return [];
  const items = dropdownEl.querySelectorAll('.grimoire-resume-item');
  return items.map((item: any) => {
    // Check direct children for content div, then find title inside
    let title = '';
    for (const child of item.children) {
      const found = child.querySelector?.('.grimoire-resume-item-title');
      if (found) {
...
describe('ResumeSessionDropdown', () => {
  ...
  describe('constructor', () => {
    ...
    it('renders empty state when no conversations', () => {
      ...
      const emptyEl = dropdownEl?.querySelector('.grimoire-resume-empty');
