import { MimocodeAgentMentionProvider } from '@/providers/mimocode/agents/MimocodeAgentMentionProvider';
import type { MimocodeAgentStorage } from '@/providers/mimocode/storage/MimocodeAgentStorage';
...
function makeMockStorage(agents: MimocodeAgentDefinition[] = []): MimocodeAgentStorage {
  return { loadAll: async () => agents } as unknown as MimocodeAgentStorage;
}

describe('MimocodeAgentMentionProvider', () => {
  it('returns an empty array before loadAgents is called', () => {
    const provider = new MimocodeAgentMentionProvider(makeMockStorage([makeAgent()]));
    expect(provider.searchAgents('')).toEqual([]);
  });

  it('returns only explicit subagents after load', async () => {
    const provider = new MimocodeAgentMentionProvider(makeMockStorage([
