/**
 * PluginManager - Discover Claude Code plugins.
 *
 ...
 *
 * Grimoire treats plugin enablement as Claude CLI-owned state. This manager is
 * read-only so the settings UI can stay a bridge instead of a partial Claude
 * settings editor.
 */
    ...
    if (installedPlugins?.plugins) {
      for (const [pluginId, entries] of Object.entries(installedPlugins.plugins)) {
        ...
        if (!Array.isArray(entries)) {
          new Notice(`Grimoire: plugin "${pluginId}" has malformed entry in installed_plugins.json (expected array, got ${typeof entries})`);
        }
