...
export interface MentionDropdownCallbacks {
  ...
  onToolMentionSelect?: (toolId: 'vault') => void;
...
export class MentionDropdownController {
  ...
  constructor(
    ...
    this.dropdown = new SelectableDropdown<MentionItem>(this.containerEl, {
      listClassName: 'grimoire-mention-dropdown',
      itemClassName: 'grimoire-mention-item',
      emptyClassName: 'grimoire-mention-empty',
      fixed: this.fixed,
      fixedClassName: 'grimoire-mention-dropdown-fixed',
    });
  }
  ...
  private hasVaultSearchMatch(searchLower: string): boolean {
    if (!searchLower.trim()) {
      return false;
    }

    const folders = this.callbacks.getCachedVaultFolders();
    const files = this.callbacks.getCachedVaultFiles();

    return folders.some((folder) => {
      const pathLower = folder.path.replace(/\\/g, '/').replace(/\/+$/, '').toLowerCase();
  ...
  private showMentionDropdown(searchText: string): void {
    ...
    const firstToolItemIndex = this.filteredMentionItems.length;
    const toolItemCount = this.appendToolItems(searchLower);
    const firstVaultItemIndex = this.filteredMentionItems.length;
    const vaultItemCount = this.appendVaultItems(searchLower);

    this.selectedMentionIndex = toolItemCount > 0
      ? firstToolItemIndex
  ...
  private appendToolItems(searchLower: string): number {
    const vaultTool = {
      type: 'tool' as const,
      id: 'vault' as const,
      name: t('chat.vaultSearch.toolName'),
      description: t('chat.vaultSearch.toolDescription'),
      insertedText: '@vault ',
    };
    const searchable = `${vaultTool.id} ${vaultTool.name} ${vaultTool.description}`.toLowerCase();

    if (!searchable.includes(searchLower)) {
      return 0;
    }

    this.filteredMentionItems.push(vaultTool);
    return 1;
  }
  ...
  private renderMentionDropdown(): void {
    this.dropdown.render({
      ...
      renderItem: (item, itemEl) => {
        ...
        switch (item.type) {
          case 'tool':
            textEl.createSpan({ cls: 'grimoire-mention-name' }).setText(`@${item.id}`);
            textEl.createSpan({ cls: 'grimoire-mention-agent-desc' }).setText(item.name);
            break;
          case 'mcp-server':
            textEl.createSpan({ cls: 'grimoire-mention-name' }).setText(`@${item.name}`);
            break;
          case 'agent-folder':
            textEl.createSpan({
              cls: 'grimoire-mention-name grimoire-mention-name-agent-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'agent': {
            // Show ID (which is namespaced for plugin agents) for consistency with inserted text
            textEl.createSpan({
              cls: 'grimoire-mention-name grimoire-mention-name-agent',
            }).setText(`@${item.id}`);
            if (item.description) {
              textEl.createSpan({ cls: 'grimoire-mention-agent-desc' }).setText(item.description);
            }
            break;
          }
          case 'context-folder':
            textEl.createSpan({
              cls: 'grimoire-mention-name grimoire-mention-name-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'context-file':
            textEl.createSpan({
              cls: 'grimoire-mention-name grimoire-mention-name-context',
            }).setText(item.name);
            break;
          case 'folder':
            textEl.createSpan({
            ...
            break;
          default:
            textEl.createSpan({ cls: 'grimoire-mention-path' }).setText(item.path || item.name);
        }
      },
  ...
  private positionFixed(): void {
    ...
    dropdownEl.setCssProps({
      '--grimoire-fixed-dropdown-bottom': `${window.innerHeight - inputRect.top + 4}px`,
      '--grimoire-fixed-dropdown-left': `${inputRect.left}px`,
      '--grimoire-fixed-dropdown-width': `${Math.max(inputRect.width, 280)}px`,
    });
  }
  ...
  private selectMentionItem(): void {
    ...
    switch (selectedItem.type) {
      ...
      case 'tool': {
        this.insertReplacement(beforeAt, selectedItem.insertedText, afterCursor);
        this.callbacks.onToolMentionSelect?.(selectedItem.id);
        break;
      }
      case 'agent-folder':
