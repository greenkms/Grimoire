...
describe('McpStorage', () => {
  ...
  describe('load/save', () => {
    it('should preserve unknown top-level keys and merge _grimoire', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
      ...
      expect(saved._grimoire).toEqual({
        extra: { keep: true },
        servers: {
          'new-server': {
            enabled: false,
    ...
    it('should keep existing _grimoire fields when metadata is defaulted', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
      ...
      const saved = JSON.parse(files.get(MCP_CONFIG_PATH) || '{}') as Record<string, unknown>;
      expect(saved._grimoire).toEqual({ extra: { keep: true } });
    ...
    it('should remove _grimoire when only servers metadata exists', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
