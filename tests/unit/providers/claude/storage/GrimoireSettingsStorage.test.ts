...
const OLD_SETTINGS_PATH = '.claude/grimoire-settings.json';
...
describe('GrimoireSettingsStorage', () => {
  let storage: GrimoireSettingsStorage;
  ...
  beforeEach(() => {
    ...
    storage = new GrimoireSettingsStorage(mockAdapter);
  });
  ...
  describe('load', () => {
    ...
    it('ignores legacy .claude settings when Grimoire settings are absent', async () => {
      mockAdapter.exists.mockImplementation(async (path: string) => (
        path === OLD_SETTINGS_PATH
      ));
      mockAdapter.read.mockImplementation(async (path: string) => {
        if (path === OLD_SETTINGS_PATH) {
    ...
    it('migrates legacy yolo permission mode to full_access', async () => {
      mockAdapter.exists.mockResolvedValue(true);
      mockAdapter.read.mockResolvedValue(JSON.stringify({
        permissionMode: 'yolo',
      ...
      expect(result.permissionMode).toBe('full_access');
      expect(result.savedProviderPermissionMode).toEqual({
        claude: 'full_access',
        codex: 'normal',
      });
      expect(mockAdapter.write).toHaveBeenCalledWith(
        GRIMOIRE_SETTINGS_PATH,
        expect.stringContaining('"permissionMode": "full_access"'),
      );
    });
    ...
    it('backs up invalid settings JSON and falls back to defaults', async () => {
      ...
      try {
        ...
        expect(mockAdapter.rename).toHaveBeenCalledWith(
          GRIMOIRE_SETTINGS_PATH,
          `${GRIMOIRE_SETTINGS_PATH}.invalid-12345`,
        );
        expect(mockAdapter.write).toHaveBeenCalledWith(
          GRIMOIRE_SETTINGS_PATH,
          expect.any(String),
    ...
    it('preserves tab counts up to 10 and normalizes tab bar placement to the fixed top layout', async () => {
      mockAdapter.exists.mockResolvedValue(true);
      mockAdapter.read.mockResolvedValue(JSON.stringify({
        maxTabs: 10,
      ...
      expect(result.maxTabs).toBe(10);
      expect(result.tabBarPosition).toBe('header');
      expect(writtenContent.maxTabs).toBe(10);
      expect(writtenContent.tabBarPosition).toBe('header');
    });
    ...
    it('normalizes advanced settings disclosure state on load', async () => {
      mockAdapter.exists.mockResolvedValue(true);
      mockAdapter.read.mockResolvedValue(JSON.stringify({
      ...
      expect(result.advancedSectionsOpen).toEqual({
        codex: false,
        general: true,
      });
      expect(writtenContent.advancedSectionsOpen).toEqual({
        codex: false,
        general: true,
      });
    });
    ...
    it('should recover from JSON parse errors', async () => {
      ...
      expect(mockAdapter.rename).toHaveBeenCalledWith(
        GRIMOIRE_SETTINGS_PATH,
        expect.stringMatching(new RegExp(`^${GRIMOIRE_SETTINGS_PATH}\\.invalid-\\d+$`)),
      );
    });
  ...
  describe('save', () => {
    ...
    it('does not delete old settings files after writing the new path', async () => {
      mockAdapter.exists.mockImplementation(async (path: string) => (
        path === OLD_SETTINGS_PATH
      ));
  ...
  describe('exists', () => {
    it('should return true when the new file exists', async () => {
      mockAdapter.exists.mockImplementation(async (path: string) => (
        path === GRIMOIRE_SETTINGS_PATH
      ));

      const result = await storage.exists();

      expect(result).toBe(true);
      expect(mockAdapter.exists).toHaveBeenCalledWith(GRIMOIRE_SETTINGS_PATH);
    });

    it('should return false when only the old settings file exists', async () => {
      mockAdapter.exists.mockImplementation(async (path: string) => (
        path === OLD_SETTINGS_PATH
      ));
      ...
      expect(mockAdapter.exists).toHaveBeenCalledWith(GRIMOIRE_SETTINGS_PATH);
      expect(mockAdapter.exists).not.toHaveBeenCalledWith(OLD_SETTINGS_PATH);
    });
