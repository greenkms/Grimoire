...
function createContainerWithInputWrapper(): { container: any; inputWrapper: any } {
  ...
  const inputWrapper = container.createDiv({ cls: 'grimoire-input-wrapper' });
...
describe('ImageContextManager', () => {
  ...
  describe('constructor with previewContainerEl', () => {
    it('should use previewContainerEl when provided', () => {
      ...
      const previewEl = previewContainer.querySelector('.grimoire-image-preview');
...
describe('ImageContextManager - Private Helpers', () => {
  ...
  describe('Paste handler', () => {
    ...
    it('paste handler should process every image item in one paste event', async () => {
      ...
      expect(addImageSpy).toHaveBeenNthCalledWith(1, firstFile, 'paste');
      expect(addImageSpy).toHaveBeenNthCalledWith(2, secondFile, 'paste');
      addImageSpy.mockRestore();
    });
  ...
  describe('Image preview rendering', () => {
    ...
    it('renderImagePreview should create chip with thumbnail, info, and remove button', () => {
      ...
      expect(chipEl.hasClass('grimoire-image-chip')).toBe(true);

      const thumbEl = chipEl.querySelector('.grimoire-image-thumb');
      expect(thumbEl).not.toBeNull();

      const infoEl = chipEl.querySelector('.grimoire-image-info');
      expect(infoEl).not.toBeNull();

      const removeEl = chipEl.querySelector('.grimoire-image-remove');
    ...
    it('remove button should delete the image and update preview', () => {
      ...
      const removeEl = firstChip.querySelector('.grimoire-image-remove');
