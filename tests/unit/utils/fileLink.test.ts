import { extractLinkTarget, hasProcessableWikilink } from '@/utils/fileLink';
...
describe('wikilink pattern matching', () => {
  describe('processable wikilink detection', () => {
    ...
    it('ignores Obsidian image embeds for chat file-link post-processing', () => {
      expect(hasProcessableWikilink('![[image.png]]')).toBe(false);
      expect(hasProcessableWikilink('Before ![[image.png|300]] after')).toBe(false);
    });

    it('still detects normal wikilinks next to image embeds', () => {
      expect(hasProcessableWikilink('![[image.png]] and [[note.md]]')).toBe(true);
    });
  });
