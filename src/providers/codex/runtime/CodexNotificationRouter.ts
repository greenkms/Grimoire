...
export class CodexNotificationRouter {
  ...
  private emitMissingAssistantSegmentText(
    ...
    if (sanitizedText) {
      this.currentAssistantSegmentText = sanitizedText;
    }
    if (!missingText) {
      return;
    }
...
function normalizeAgentMessageCompletionText(
  ...
  if (streamedAssistantText.startsWith(text) && streamedAssistantText.slice(text.length).trim().length === 0) {
    return '';
  }
  const streamedWithoutTrailingWhitespace = streamedAssistantText.trimEnd();
  if (streamedWithoutTrailingWhitespace && text.startsWith(streamedWithoutTrailingWhitespace)) {
    return text.slice(streamedWithoutTrailingWhitespace.length);
  }
