...
describe('ExternalContextSelector', () => {
  ...
  it('should keep the Files dropdown inside the composer bounds when near the right edge', () => {
    const composerEl = createMockEl();
    composerEl.getBoundingClientRect = jest.fn(() => ({
      bottom: 760,
      height: 170,
      left: 100,
      right: 360,
    ...
    const container = parentEl.querySelector('.grimoire-external-context-selector');
    container!.closest = jest.fn((selector: string) =>
      selector === '.grimoire-composer-shell' ? composerEl : null
    );
    ...
    container!.ownerDocument.defaultView.innerWidth = 430;
    container!.ownerDocument.defaultView.innerHeight = 800;

    container!.dispatchEvent('mouseenter');

    const dropdown = parentEl.querySelector('.grimoire-external-context-dropdown');
    expect(dropdown?.style['--grimoire-external-context-dropdown-left']).toBe('110px');
    expect(dropdown?.style['--grimoire-external-context-dropdown-width']).toBe('240px');
    expect(dropdown?.style['--grimoire-external-context-dropdown-bottom']).toBe('88px');
  });
