...
describe('Sync Subagent Renderer', () => {
  ...
  describe('createSubagentBlock', () => {
    ...
    it('should not show a tool count badge in the header', () => {
      ...
      expect(getTextByClass(state.wrapperEl as any, 'grimoire-subagent-count')).toEqual([]);
    });
  });
  ...
  describe('renderStoredSubagent', () => {
    ...
    it('should toggle expand/collapse on click', () => {
      ...
      expect(contentEl.hasClass('grimoire-hidden')).toBe(false);
...
describe('Async Subagent Renderer', () => {
  ...
  it('shows prompt in content and keeps label visible while running', () => {
    ...
    const contentText = getTextByClass(state.contentEl as any, 'grimoire-subagent-prompt-text')[0];
  ...
  it('finalizes to completed and reveals description', () => {
    ...
    const contentText = getTextByClass(state.contentEl as any, 'grimoire-subagent-result-output')[0];
  ...
  describe('renderStoredAsyncSubagent', () => {
    it('should return wrapper element', () => {
      ...
      expect((wrapperEl as any).hasClass('grimoire-subagent-list')).toBe(true);
    });
    ...
    it('renders error status correctly', () => {
      ...
      const contentText = getTextByClass(wrapperEl as any, 'grimoire-subagent-result-output')[0];
    ...
    it('renders running status with prompt', () => {
      ...
      const contentText = getTextByClass(wrapperEl as any, 'grimoire-subagent-prompt-text')[0];
...
describe('addSubagentToolCall', () => {
  ...
  it('merges repeated tool IDs instead of duplicating tool rows', () => {
    ...
    expect(getTextByClass(state.wrapperEl as any, 'grimoire-subagent-count')).toEqual([]);
    expect(getTextByClass(state.toolsContainerEl as any, 'grimoire-subagent-tool-name')).toEqual(['Write']);
    expect(getTextByClass(state.toolsContainerEl as any, 'grimoire-subagent-tool-summary')).toEqual(['notes.md']);
  });
});
...
describe('finalizeSubagentBlock', () => {
  ...
  it('keeps tool history and shows result section text', () => {
    ...
    const doneText = getTextByClass(state.contentEl as any, 'grimoire-subagent-result-output')[0];
  ...
  it('shows ERROR text when isError is true', () => {
    ...
    const errorText = getTextByClass(state.contentEl as any, 'grimoire-subagent-result-output')[0];
...
describe('renderStoredSubagent status variants', () => {
  ...
  it('renders completed subagent with done class and check icon', () => {
    ...
    expect(setIcon).toHaveBeenCalledWith(expect.anything(), 'check');
    const doneText = getTextByClass(wrapperEl as any, 'grimoire-subagent-result-output')[0];
    expect(doneText).toBe('DONE');
  });
  ...
  it('renders error subagent with error class and x icon', () => {
    ...
    expect((wrapperEl as any).hasClass('error')).toBe(true);
    expect(setIcon).toHaveBeenCalledWith(expect.anything(), 'x');
    const errorText = getTextByClass(wrapperEl as any, 'grimoire-subagent-result-output')[0];
  ...
  it('renders running subagent tool call with expanded-style result', () => {
    ...
    const resultTexts = getTextByClass(contentEl, 'grimoire-tool-line');
  ...
  it('does not render a tool count badge for stored subagents', () => {
    ...
    expect(getTextByClass(wrapperEl as any, 'grimoire-subagent-count')).toEqual([]);
  });
  ...
  it('truncates long descriptions', () => {
    ...
    const labelTexts = getTextByClass(wrapperEl as any, 'grimoire-subagent-label');
